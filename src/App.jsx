...
export default function App() {
  ...
  const addMeal = () => {
    ...
    setMeals([...meals, { id: Date.now(), name: mealName, cal: Number(mealCal), time: new Date().toTimeString().slice(0, 5), color: colors[meals.length % colors.length] }]);
  ...
  const inp = {
    border: "2px solid #F0F0F0", borderRadius: 12, padding: "12px 14px",
  ...
  const btn = (c) => ({
    ...
    padding: "14px 24px", fontWeight: "bold", fontSize: 16, cursor: "pointer",
  ...
  return (
    <div style={{ fontFamily: "'Segoe UI','Hiragino Sans',sans-serif", background: "#FFF9F0", minHeight: "100vh", maxWidth: 480, margin: "0 auto" }}>
            ...
            {meals.map((m) => (
              <div key={m.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 0", borderBottom: "1px solid #F5F5F5" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  <button onClick={() => setMeals(meals.filter((x) => x.id !== m.id))} style={{ background: "none", border: "none", color: "#CCC", cursor: "pointer", fontSize: 18, padding: "4px 8px", touchAction: "manipulation" }}>×</button>
                </div>
              </div>
            ))}
        ...
        {tab === 2 && <>
          ...
          <div style={card}>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <input style={{ ...inp, flex: 1 }} placeholder="例: 66.5" inputMode="decimal" pattern="[0-9]*\.?[0-9]*" value={weightInput} onChange={(e) => setWeightInput(e.target.value.replace(/[^0-9.]/g, ""))} />
              <button onClick={addWeight} style={{ ...btn("#4D96FF"), width: "auto", whiteSpace: "nowrap", padding: "14px 20px" }}>記録</button>
            </div>
          </div>
          <div style={card}>
      ...
      <div style={{ position: "fixed", bottom: 0, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 480, background: "#fff", display: "flex", borderTop: "1px solid #F0F0F0", boxShadow: "0 -4px 20px rgba(0,0,0,0.08)", zIndex: 100 }}>
        {TABS.map((icon, i) => (
          <button key={i} onClick={() => setTab(i)} style={{ flex: 1, padding: "10px 4px 8px", border: "none", background: "none", fontSize: 11, fontWeight: i === tab ? "bold" : "normal", color: i === tab ? "#FF6B6B" : "#888", cursor: "pointer", WebkitTapHighlightColor: "transparent", touchAction: "manipulation" }}>
...
function WeightGraph({ entries }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", borderRadius: 16 }}>
      ...
      <polygon points={`${x(0)},${H - pad} ${points} ${x(entries.length - 1)},${H - pad}`}